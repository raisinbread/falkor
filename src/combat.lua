...
-- Create alias: combatstart (enable combat tracking)
Falkor:registerAlias("aliasCombatStart", "^combatstart$", [[
    Falkor:startCombatTracking()
]])

-- Create alias: combatstop (disable combat tracking)
Falkor:registerAlias("aliasCombatStop", "^combatstop$", [[
    Falkor:stopCombatTracking()
]])
...
-- Create alias: fstophunt (stop hunting)
Falkor:registerAlias("aliasStopHunt", "^fstophunt$", [[
    Falkor:stopHunting()
]])

-- Create alias: fcombat (display current denizens)
Falkor:registerAlias("aliasFcombat", "^fcombat$", [[
    -- Manually read current GMCP data and display denizens
    if not gmcp or not gmcp.Char or not gmcp.Char.Items or not gmcp.Char.Items.List then
        Falkor:log("<yellow>No GMCP Items.List data available")
        return
    end
    ...
    -- Extract denizens from current items
    if itemData.items then
        for _, item in ipairs(itemData.items) do
            if Falkor:checkIsDenizen(item) then
                table.insert(denizens, {
                    id = item.id,
                    name = item.name,
                    attrib = item.attrib,
                    icon = item.icon or "none"
    ...
    -- Display denizens
    Falkor:log("<cyan>=== Denizens in Room [Manual] ===")
    
    if #denizens == 0 then
        Falkor:log("<yellow>No denizens in room")
    else
        Falkor:log(string.format("<green>Denizen count: <white>%d", #denizens))
        
        -- Sort by name
        table.sort(denizens, function(a, b)
            return a.name < b.name
        end)
        
        -- Log each denizen
        for i, denizen in ipairs(denizens) do
            Falkor:log(string.format(
                "<white>[%d] <yellow>%s <gray>(ID: %s, Icon: %s, Attrib: %s)",
                i,
                denizen.name,
                denizen.id,
                denizen.icon,
